...
public:
    ...
    PetscErrorCode InitializeComponents() {
        ...
        if (config_implicit_source) source = std::make_unique<SourceStep<Real>>(dmQ, dmAux, parameters);
    ...
    PetscErrorCode Run(int argc, char **argv) override {
...

        {
            ...
            if (!settings.io.initial_condition_file.empty()) {
                ...
                PetscCall(io->LoadSolution(X, dmQ, settings.io.initial_condition_file, mask));
    ...
    static PetscErrorCode RHSWrapper(TS ts, PetscReal t, Vec X, Vec F, void* ctx) { return ((ModularSolver*)ctx)->transport->FormRHS(t, X, F); }
    static PetscErrorCode SplittingWrapper(TS ts) {
        ...
        if (solver->source) { 
            PetscReal dt; 
            TSGetTimeStep(ts, &dt); 
            // Call local solve, ensuring return code is checked
