...
public:
    ...
    PetscErrorCode Run(int argc, char **argv) override {
...

        {
            ...
            // Initial update to ensure consistency before loading file
            ...
            if (!init_file.empty()) {
                if (init_file.find(".msh") != std::string::npos) {
                    ...
                    for(PetscInt c=cStart; c<cEnd; ++c) {
                        ...
                        // --- SAFETY CHECK 1: Q Vector Bounds ---
                        ...
                        // --- SAFETY CHECK 2: Aux Vector Bounds & Update ---
                        if (offQ >= 0 && (offQ + n_dof) <= loc_size_Q &&
